...
const voiceChannelPresence = new Map();
const presenceSocketCounts = new Map();
const pendingOfflinePresenceTimers = new Map();
const PRESENCE_OFFLINE_GRACE_MS = Math.max(800, Number(process.env.PRESENCE_OFFLINE_GRACE_MS || 4000));
const getPresenceSocketCount = (userId) => presenceSocketCounts.get(userId) ?? 0;
const incrementPresenceSocketCount = (userId) => {
    const nextCount = getPresenceSocketCount(userId) + 1;
    presenceSocketCounts.set(userId, nextCount);
    return nextCount;
};
const decrementPresenceSocketCount = (userId) => {
...
const clearPendingOfflinePresence = (userId) => {
...
const normalizePermissionList = (value) => {
    ...
    for (const entry of list) {
        if (typeof entry !== 'string')
            continue;
        if (!VALID_PERMISSIONS.has(entry))
            continue;
        out.add(entry);
    }
    return Array.from(out);
};
const normalizePermissionOverwrites = (value) => {
    const list = Array.isArray(value) ? value : [];
    const out = [];
    for (const entry of list) {
        ...
        out.push({
            id,
            type,
            allow: normalizePermissionList(entry.allow),
            deny: normalizePermissionList(entry.deny),
        });
    }
    return out;
};
const addVoicePresence = (channelId, userId) => {
    const userCounts = voiceChannelPresence.get(channelId) ?? new Map();
    const prevCount = userCounts.get(userId) ?? 0;
    userCounts.set(userId, prevCount + 1);
    voiceChannelPresence.set(channelId, userCounts);
    return prevCount === 0;
};
const removeVoicePresence = (channelId, userId) => {
    ...
    if (userCounts.size === 0)
        voiceChannelPresence.delete(channelId);
    return !userCounts.has(userId);
};
const isUserPresentInVoiceChannel = (channelId, userId) => {
    const userCounts = voiceChannelPresence.get(channelId);
    if (!userCounts)
        return false;
    return (userCounts.get(userId) ?? 0) > 0;
};
const getVoiceSnapshotChannels = () => Array.from(voiceChannelPresence.entries()).map(([channelId, userCounts]) => ({
    channelId,
    userIds: Array.from(userCounts.keys()),
}));
const getVoiceUserIdsForChannel = (channelId) => {
    const userCounts = voiceChannelPresence.get(channelId);
    if (!userCounts)
        return [];
...
const normalizeChannelAttachments = (value) => {
    ...
    for (const entry of value) {
        ...
        if (!id || !rawUrl || rawUrl.startsWith('data:'))
            continue;
        if (rawUrl.length > MAX_CHANNEL_ATTACHMENT_URL_LENGTH)
            continue;
        ...
        if (output.length >= MAX_CHANNEL_ATTACHMENTS)
            break;
    }
    return output.length > 0 ? output : undefined;
};
const normalizeChannelMessage = (value, channelId, fallbackAuthorId) => {
    if (!value || typeof value !== 'object')
        return null;
    const id = typeof value.id === 'string' ? value.id.trim().slice(0, 128) : '';
    if (!id)
        return null;
    const authorId = typeof value.authorId === 'string' && value.authorId.trim().length > 0
        ? value.authorId.trim().slice(0, 128)
        : fallbackAuthorId;
    const content = typeof value.content === 'string' ? value.content.slice(0, MAX_CHANNEL_CONTENT_LENGTH) : '';
    const timestamp = typeof value.timestamp === 'string' && value.timestamp.trim().length > 0
        ? value.timestamp
        : new Date().toISOString();
    const attachments = normalizeChannelAttachments(value.attachments);
...
const findServerAndChannelById = (servers, channelId) => {
...
const getChannelMemberPermissions = (server, channel, userId) => {
    if (!server || !channel || !userId)
        return new Set();
    ...
    for (const overwrite of overwrites) {
        if (!overwrite || overwrite.type !== 'role' || !roleIds.has(overwrite.id))
            continue;
    ...
    const userOverwrite = overwrites.find((overwrite) => overwrite?.type === 'member' && overwrite?.id === userId);
    if (userOverwrite) {
...
const hasChannelPermission = (server, channel, userId, permission) => {
...
const hasChannelViewPermission = (server, channel, userId) => {
    return hasChannelPermission(server, channel, userId, 'VIEW_CHANNEL');
};
const getVoiceChannelRules = async (channelId, userId) => {
    try {
...
const enqueueSnapshotMutation = (mutator) => {
    snapshotQueue = snapshotQueue
        ...
        try {
            ...
            const base = row?.data && typeof row.data === 'object' && !Array.isArray(row.data)
...
export function createSocketServer(httpServer) {
    ...
    const emitVoiceChannelSync = (channelId) => {
        io.emit('voice:sync', {
            channelId,
    ...
    io.on('connection', (socket) => {
        ...
        const socketVoiceChannels = new Set();
        ...
        socket.on('channel:create', (payload) => {
            if (!payload?.serverId || !payload?.categoryId || !payload?.channel)
                return;
            socket.broadcast.emit('channel:create', payload);
            enqueueSnapshotMutation((state) => {
                if (!Array.isArray(state.servers))
                    return state;
                const server = state.servers.find((s) => s?.id === payload.serverId);
                if (!server || !Array.isArray(server.categories))
                    return state;
                const category = server.categories.find((c) => c?.id === payload.categoryId);
                if (!category)
                    return state;
                if (!Array.isArray(category.channels))
                    category.channels = [];
                const exists = category.channels.some((ch) => ch?.id === payload.channel.id);
                if (!exists)
                    category.channels.push(payload.channel);
                return state;
            });
        });
        socket.on('channel:update', (payload) => {
            const serverId = payload?.serverId;
            const channelId = payload?.channelId;
            const updates = payload?.updates;
            if (!serverId || !channelId || !updates || typeof updates !== 'object')
                return;
            const nextUpdates = {};
            if (typeof updates.name === 'string') {
                const normalized = updates.name.trim().toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-_]/g, '');
                if (normalized.length >= 2)
                    nextUpdates.name = normalized;
            }
            ...
            if (updates.userLimit !== undefined) {
                ...
                const normalizedLimit = Number.isFinite(parsed) && parsed > 0
            ...
            enqueueSnapshotMutation((state) => {
                ...
                for (const server of state.servers) {
                    if (!server || server.id !== serverId || !Array.isArray(server.categories))
                        continue;
                    for (const category of server.categories) {
                        if (!category || !Array.isArray(category.channels))
                            continue;
        ...
        socket.on('channel:message:update', (payload) => {
            if (!payload?.channelId || !payload?.messageId || !payload?.updates)
                return;
            ...
            enqueueSnapshotMutation((state) => {
                ...
                if (idx === -1)
                    return state;
                const editedAt = payload.updates?.editedAt ||
        ...
        socket.on('channel:message:delete', async (payload) => {
            ...
            try {
                ...
                const targetMessage = channelMessages.find((entry) => entry?.id === payload.messageId);
                if (!targetMessage)
                    return;
            ...
            enqueueSnapshotMutation((state) => {
                ...
                const current = Array.isArray(state.messages[payload.channelId]) ? state.messages[payload.channelId] : [];
                state.messages[payload.channelId] = current.filter((m) => m?.id !== payload.messageId);
                return state;
            });
        });
        socket.on('channel:message:reactions', (payload) => {
        ...
        socket.on('channel:message:pin', (payload) => {
            if (!payload?.channelId || !payload?.messageId || typeof payload.isPinned !== 'boolean')
                return;
        ...
        socket.on('dm:request:accept', (payload) => {
        ...
        socket.on('dm:request:reject', (payload) => {
        ...
        socket.on('voice:join', async (payload) => {
            ...
            if (!channelId)
                return;
            if (payload?.userId && payload.userId !== user.userId)
                return;
        ...
        socket.on('voice:update', (payload) => {
            if (payload?.userId && payload.userId !== user.userId)
                return;
            const updates = {};
            if (typeof payload?.updates?.muted === 'boolean')
                updates.muted = payload.updates.muted;
            if (typeof payload?.updates?.deafened === 'boolean')
                updates.deafened = payload.updates.deafened;
        ...
        socket.on('webrtc:ice-candidate', (payload) => {
            const channelId = payload?.channelId;
            const toUserId = payload?.toUserId;
            if (!channelId || !toUserId || !payload?.candidate)
                return;
            if (toUserId === user.userId)
                return;
            const fromPresent = isUserPresentInVoiceChannel(channelId, user.userId);
        ...
        socket.on('disconnect', () => {
            ...
            if (remainingSockets <= 0) {
                ...
                const timer = setTimeout(() => {
                    ...
                    if (getPresenceSocketCount(user.userId) > 0)
                        return;
