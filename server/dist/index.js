...
app.use((req, res, next) => {
    ...
    res.setHeader('Permissions-Policy', 'camera=(self), microphone=(self), geolocation=(), payment=(), usb=(), midi=(), accelerometer=(), gyroscope=()');
...
const BOOTSTRAP_DM_MESSAGES_PER_CONVERSATION = Number(process.env.BOOTSTRAP_DM_MESSAGES_PER_CONVERSATION || 120);
...
const invalidateBootstrapCache = (userIds) => {
...
app.use('/media/chat', express.static(mediaChatDir, {
    fallthrough: true,
    maxAge: '30d',
    setHeaders: (res) => {
...
const getErrorCode = (error) => typeof error === 'object' && error && 'code' in error && typeof error.code === 'string'
    ? String(error.code)
    : '';
const getErrorMessage = (error) => typeof error === 'object' && error && 'message' in error && typeof error.message === 'string'
...
const isDatabaseUnavailableError = (error) => {
    ...
    return /p1001|p1002|can't reach database server|timed out|econnrefused|econnreset|enotfound|prismaclientinitializationerror|data transfer quota|connection limit|database is unavailable|no pg_hba\.conf entry|too many connections/.test(details);
};
const getDatabaseErrorDiagnosis = (error) => {
    const code = getErrorCode(error);
    const message = redactSensitive(getErrorMessage(error));
    const details = `${code} ${message}`.toLowerCase();
    if (/p1001|p1002|can't reach database server|timed out|econnrefused|econnreset|enotfound|no pg_hba\.conf entry|too many connections|database is unavailable|data transfer quota|connection limit/.test(details)) {
        return {
            kind: 'unavailable',
            code,
...
const normalizeInviteCode = (value) => value.trim().toLowerCase();
const normalizeSnapshotInvite = (raw) => {
...
let ffmpegProbeCached = null;
const isFfmpegAvailable = () => {
    if (ffmpegProbeCached !== null)
...
const normalizeMediaFilename = (input, fallback = 'clip') => {
...
const normalizeUploadExtension = (filename, contentType) => {
    ...
    if (mime.includes('image/gif'))
        return '.gif';
    if (mime.includes('image/svg'))
        return '.svg';
    if (mime.includes('video/mp4'))
        return '.mp4';
    if (mime.includes('video/webm'))
        return '.webm';
    if (mime.includes('video/quicktime'))
        return '.mov';
    if (mime.includes('audio/webm'))
        return '.webm';
    if (mime.includes('audio/wav') || mime.includes('audio/wave') || mime.includes('audio/x-wav'))
        return '.wav';
    if (mime.includes('audio/ogg'))
        return '.ogg';
    if (mime.includes('audio/opus'))
        return '.opus';
    if (mime.includes('audio/mp4'))
        return '.m4a';
    if (mime.includes('audio/x-m4a'))
        return '.m4a';
    if (mime.includes('audio/mpeg'))
        return '.mp3';
    return '';
};
const shouldTranscodeAudioForMobile = (contentType, filename) => {
    const mime = (contentType || '').toLowerCase();
    const ext = path.extname(filename || '').toLowerCase();
    const lowerFilename = String(filename || '').toLowerCase();
    const audioHintInFilename = /(^|[_\-.])(voice|audio|record|rec|mic|memo|mensaje|nota)([_\-.]|$)/.test(lowerFilename);
    if (mime.startsWith('audio/')) {
        if (mime.includes('mpeg') || mime.includes('mp4') || mime.includes('aac'))
            return false;
        return true;
    }
    ...
    if (mime === 'application/octet-stream') {
        if (audioHintInFilename)
            return true;
        return ['.wav', '.wave', '.webm', '.ogg', '.oga', '.opus', '.flac', '.aif', '.aiff'].includes(ext);
    }
    return false;
};
const transcodeAudioToM4a = async (inputBuffer, filenameHint) => {
...
const findInviteInState = (state, inviteCode) => {
    ...
    for (let i = 0; i < servers.length; i += 1) {
        ...
        for (let j = 0; j < invites.length; j += 1) {
            ...
            if (!normalizedInvite)
                continue;
            if (normalizedInvite.code !== inviteCode)
                continue;
...
const normalizeDmAttachments = (value) => {
...
const normalizeStateAttachment = (value) => {
    if (!value || typeof value !== 'object')
        return null;
    const id = typeof value.id === 'string' ? value.id.slice(0, 128) : '';
    const url = typeof value.url === 'string' ? value.url.slice(0, MAX_STATE_ATTACHMENT_URL_LENGTH) : '';
    const filename = typeof value.filename === 'string'
        ? value.filename.slice(0, MAX_STATE_ATTACHMENT_FILENAME_LENGTH)
        : '';
    const contentType = typeof value.contentType === 'string'
        ? value.contentType.slice(0, MAX_STATE_ATTACHMENT_CONTENT_TYPE_LENGTH)
        : '';
    const size = Number(value.size);
    if (!id || !url || !filename || !contentType)
        return null;
...
const sanitizeStateMessage = (value) => {
    if (!value || typeof value !== 'object')
        return null;
    const id = typeof value.id === 'string' ? value.id.slice(0, 128) : '';
    const channelId = typeof value.channelId === 'string' ? value.channelId.slice(0, 128) : '';
    const authorId = typeof value.authorId === 'string' ? value.authorId.slice(0, 128) : '';
    const timestamp = typeof value.timestamp === 'string' && value.timestamp.trim().length > 0
        ? value.timestamp
        : new Date().toISOString();
    const content = typeof value.content === 'string' ? value.content.slice(0, MAX_STATE_CONTENT_LENGTH) : '';
    const rawAttachments = Array.isArray(value.attachments) ? value.attachments : [];
    const attachments = rawAttachments
        .map((attachment) => normalizeStateAttachment(attachment))
    ...
    return {
        ...
        content,
        timestamp,
        editedAt: typeof value.editedAt === 'string' ? value.editedAt : undefined,
        isPinned: Boolean(value.isPinned),
        replyToId: typeof value.replyToId === 'string' ? value.replyToId.slice(0, 128) : undefined,
        threadId: typeof value.threadId === 'string' ? value.threadId.slice(0, 128) : undefined,
        reactions: Array.isArray(value.reactions) ? value.reactions : undefined,
        attachments: attachments.length > 0 ? attachments : undefined,
    };
};
const sanitizeStateMessageMap = (value, perChannelLimit) => {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return {};
    const safe = {};
    for (const [channelId, messages] of Object.entries(value)) {
        if (!Array.isArray(messages) || messages.length === 0)
            continue;
        const next = messages
            .map((message) => sanitizeStateMessage(message))
...
const sanitizeAppState = (raw, scope) => {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return null;
    const state = raw;
    const safeServers = Array.isArray(state.servers) ? state.servers : [];
    const safeActiveServerId = typeof state.activeServerId === 'string' || state.activeServerId === null ? state.activeServerId : null;
    const safeActiveChannelId = typeof state.activeChannelId === 'string' || state.activeChannelId === null ? state.activeChannelId : null;
    if (scope === 'servers') {
        return {
            servers: safeServers,
    ...
    return {
        ...
        presences: state.presences && typeof state.presences === 'object' && !Array.isArray(state.presences)
            ? state.presences
            : {},
        activeServerId: safeActiveServerId,
        activeChannelId: safeActiveChannelId,
        memberTimeouts: state.memberTimeouts && typeof state.memberTimeouts === 'object' && !Array.isArray(state.memberTimeouts)
            ? state.memberTimeouts
            : {},
        serverBans: state.serverBans && typeof state.serverBans === 'object' && !Array.isArray(state.serverBans)
            ? state.serverBans
            : {},
        auditLog: state.auditLog && typeof state.auditLog === 'object' && !Array.isArray(state.auditLog)
            ? state.auditLog
            : {},
        ...
        activeThreadId: typeof state.activeThreadId === 'string' || state.activeThreadId === null ? state.activeThreadId : null,
    };
};
...
const normalizeRecoveryCode = (value) => value.trim().toUpperCase();
...
app.post('/media/upload', mediaRateLimit, requireAuth, express.raw({ type: '*/*', limit: MEDIA_UPLOAD_LIMIT }), asyncRoute(async (req, res) => {
...
app.patch('/me/profile', requireAuth, asyncRoute(async (req, res) => {
    ...
    if (parsed.data.displayName !== undefined)
        data.displayName = normalizeText(parsed.data.displayName);
    if (parsed.data.pronouns !== undefined)
        data.pronouns = normalizeText(parsed.data.pronouns);
    if (parsed.data.bio !== undefined)
        data.bio = normalizeText(parsed.data.bio);
    if (parsed.data.avatar !== undefined)
        data.avatar = normalizeText(parsed.data.avatar);
    if (parsed.data.banner !== undefined)
        data.banner = normalizeText(parsed.data.banner);
...
app.get('/state/app', requireAuth, asyncRoute(async (req, res) => {
    const scope = req.query.scope === 'servers' ? 'servers' : 'full';
...
app.post('/invites/:code/join', requireAuth, asyncRoute(async (req, res) => {
    ...
    const status = getInviteStatus(found.invite);
    if (status !== 'valid')
        return res.status(400).json({ error: `invite_${status}` });
...
app.post('/dm/requests/:id/accept', requireAuth, asyncRoute(async (req, res) => {
    ...
    const conversation = existingConversation && existingConversation.members.length === 2
